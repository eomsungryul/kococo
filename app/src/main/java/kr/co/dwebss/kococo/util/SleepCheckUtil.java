		...
		if (decibel > chkOSADb) {
			...
			if (isOSATerm == true) {
				if (beforeTermWord.equals(BREATH) && isOSATermCnt > 500) {
					...
					AudioAnalysisUtil.osaTermList.add(new StartEnd());
					AudioAnalysisUtil.osaTermList.get(AudioAnalysisUtil.osaTermList.size()-1).setStart(OSAcurTermTime);
					AudioAnalysisUtil.osaTermList.get(AudioAnalysisUtil.osaTermList.size()-1).setEnd(times);
